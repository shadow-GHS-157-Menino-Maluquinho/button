...
var hold_time = 1 # Tempo necessário para completar o círculo (em segundos)
...
func _draw():
	if progress > 0:  # Desenha o arco apenas se o progresso for maior que 0
		$TextureProgressBar.value = current_progress_bar_value * 50 # Ajusta para a escala de 0-100 (se necessário)

# Função para mudar a cena
func _change_scene():
	get_tree().change_scene_to_file("res://node_3d.tscn")  # Muda para a nova cena carregada
